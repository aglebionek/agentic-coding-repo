---
name: js-to-ts-report
description: Analyzes JavaScript or JSX targets and their coupled surfaces for a safe, guideline-compliant TypeScript conversion. Use only when the user explicitly invokes "js-to-ts report" with target files or symbols.
---

# JS to TS Report

Produce a read-only, portable report. Do not modify code, types, tests, configuration, snapshots, generated artifacts, or the Git index. Emit it in conversation unless the user supplies a report path.
