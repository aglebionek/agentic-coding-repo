---
name: js-to-ts-autoscope-report
description: Expands a JS/JSX seed into one bounded, guideline-aware TypeScript conversion spine. Use only when the user invokes "js-to-ts-autoscope report <seed-file-or-small-seed-set> [deep|wide]".
---

# JS to TS Autoscope Report

Produce a read-only, portable report. Do not modify source, tests, configuration, documentation, generated artifacts, snapshots, dependencies, or the Git index. Emit it in conversation unless the user supplies a report path.
...
2. Reject `current changes`, `codebase`, broad directories such as `instago-app/server`, generated/vendor/static/template-only targets, and unsupported files with `Valid: false`.
...
5. Read applicable `AGENTS.md`, root `CODING_GUIDELINES.md`, package/tooling configuration, and the existing JS-to-TS skill only as compatibility context.
