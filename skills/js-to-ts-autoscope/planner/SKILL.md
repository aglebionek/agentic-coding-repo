---
name: js-to-ts-autoscope-planner
description: Creates one read-only oneshot or sequenced TypeScript conversion plan from a valid js-to-ts-autoscope report. Use only when the user invokes "js-to-ts-autoscope planner" or "js-to-ts-autoscope autoplan" with a report.
---

# JS to TS Autoscope Planner

Produce a plan, not changes. Do not modify source, tests, configuration, documentation, generated artifacts, snapshots, dependencies, or the Git index. Emit it in conversation unless the user supplies a plan path.
...
2. Reject implementation planning when `Valid: false`.
   ...
   - In `planner`, optionally read [grill-me](../../grill-me/SKILL.md) and resolve recovery decisions, but output only a clarified report-rerun handoff.
...
- `js-to-ts-autoscope planner`: interactive. Read [grill-me](../../grill-me/SKILL.md), resolve material decisions one at a time, and recommend an answer.
- `js-to-ts-autoscope autoplan`: AFK-safe. Make only evidence-backed decisions. Reject instead of guessing through purpose ambiguity, behavior/product/security/public-contract decisions, migrations, new dependencies, destructive work, architecture rewrites, or invalid reports.
...
6. Add or update tests when needed to prove the spine invariant, result variants, contract refactors, or behavior risk. Do not add tests merely because the selected spine lacked tests.
...
Keep execution, approval loops, runners, code-workflow integration, commits, and pull requests outside this skill.
